...
BOOL WriteBenchmarkFile()
{
	...
	GenerateHexIntegers_KB(writeData);
	clock_t end_1 = clock();
	...
	if (dwTotalFileSize.QuadPart == BENCHMARK__FILE_SIZE)
	{
		...
		//_tprintf(TEXT("\tTest file name: \"%s\";\n"), 
		//	((BENCHMARK__FILE_NAME).begin(), (BENCHMARK__FILE_NAME).end()).c_str());
		_tprintf(TEXT("\tTest file size: %d (bytes);\n"), BENCHMARK__FILE_SIZE);
		_tprintf(TEXT("\tData generation time: %.4lf sec;\n"), elapsed_secs_dataGen);
		_tprintf(TEXT("\tData file write time: %.4lf sec;\n"), elapsed_secs_dataWrite);
		return TRUE;
	}
	else
	{
...
inline void CurrentDriveBenchmark()
{
	...
	if (WriteBenchmarkFile())
	{
		...
		_tprintf(TEXT("\tData file read time: %.4lf sec.\n"), elapsed_secs_read);
